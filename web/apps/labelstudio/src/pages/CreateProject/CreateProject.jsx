    ...
    >
      <div className="w-full flex flex-col gap-2">
        <label className="w-full" htmlFor="project_name">
          {t("createProject.form.nameLabel")}
        </label>
        <Input
          name="name"
          id="project_name"
      ...
      <div className="w-full flex flex-col gap-2">
        <label className="w-full" htmlFor="project_description">
          {t("createProject.form.descriptionLabel")}
        </label>
        <TextArea
          name="description"
      ...
      {isFF(FF_LSDV_E_297) && (
        <div className="w-full flex flex-col gap-2">
          <label>
            {t("createProject.form.workspaceLabel")}
            <EnterpriseBadge className="ml-2" />
          </label>
          <Select
            placeholder={t("createProject.form.workspacePlaceholder") ?? undefined}
            disabled
            options={[]}
            triggerClassName="!flex-1"
          />
          <Typography size="small" className="mt-tight mb-wider">
            {t("createProject.form.workspaceHelper")}{" "}
            <a
              href={createURL(
            ...
            >
              {t("actions.learnMore", { defaultValue: t("createProject.form.workspaceLearnMore") })}
            </a>
          </Typography>
  ...
  return (
    <Modal onHide={onDelete} closeOnClickOutside={false} allowToInterceptEscape fullscreen visible bare>
      <div className={rootClass}>
        <Modal.Header>
          <h1>{t("createProject.modal.title")}</h1>
          ...
          <Space>
            <Button
              ...
              onClick={onDelete}
              waiting={waiting}
              aria-label={t("createProject.modal.cancelAria")}
            >
              {t("actions.cancel", { defaultValue: t("createProject.modal.cancel") })}
            </Button>
            <Button
              look="primary"
              size="small"
            ...
            >
              {t("actions.save", { defaultValue: t("createProject.modal.save") })}
            </Button>
          </Space>
        </Modal.Header>
        <ProjectName
