...
export const EmptyProjectsList = ({ openModal }) => {
  ...
  return (
    <Block name="empty-projects-page">
      ...
      <Button onClick={openModal} className="my-8" aria-label={t("projects.card.createAria")}>
  ...
  return (
    <Elem tag={NavLink} name="link" to={`/projects/${project.id}/data`} data-external>
      <Block name="project-card" mod={{ colored: !!color }} style={projectColors}>
        <Elem name="header">
          <Elem name="title">
            <Elem name="title-text">{project.title ?? t("projects.card.defaultTitle")}</Elem>
            ...
            >
              <Dropdown.Trigger
                content={
                  <Menu contextual>
                    <Menu.Item href={`/projects/${project.id}/settings`}>{t("projects.card.menu.settings")}</Menu.Item>
                    <Menu.Item href={`/projects/${project.id}/data?labeling=1`}>
                      {t("projects.card.menu.label")}
                    </Menu.Item>
                  </Menu>
                }
              >
                <Button size="smaller" look="string" aria-label={t("projects.card.menu.ariaLabel")}>
                  <IconEllipsis />
                </Button>
              </Dropdown.Trigger>
            </Elem>
